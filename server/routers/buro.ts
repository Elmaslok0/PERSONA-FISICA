...
export const buroRouter = router({
  ...
  /**
   * Obtiene el informe completo del Buró integrando todas las APIs
   */
  obtenerInforme: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        // Ejecutar todas las consultas en paralelo, incluyendo las nuevas APIs
        const [
          prospectorData, 
          estimadorData, 
          informeBuroData, 
          ...
          buroAPIClient.consultarMonitor(input.persona),
          buroAPIClient.obtenerReporteCredito(input.persona),
        ]);
        ...
        await updateBuroConsultation(input.consultationId, {
          ...
          creditReportData: JSON.stringify(reporteCreditoData),
        ...
        return {
          ...
          monitor: monitorData,
          reporteCredito: reporteCreditoData,
        };
      } catch (error) {
    ...
    .query(async ({ ctx, input }) => {
      try {
        ...
        return {
          ...
          monitorData: (consultation as any).monitorData ? JSON.parse((consultation as any).monitorData) : null,
          creditReportData: (consultation as any).creditReportData ? JSON.parse((consultation as any).creditReportData) : null,
        };
      } catch (error) {
